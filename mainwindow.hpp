...
private slots:
    ...
    void disableSomeActions(bool disable);
    /** Производит соединение текущего сигнала selectionModel и слота disabledisableDeleteAction
     *
     * Каждая записная книжка имеет свой selectionModel, который меняется при открытии
     * других записных книг. Таким образом, как только была открыта книга, необходимо
